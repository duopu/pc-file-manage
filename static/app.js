...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape' && !mediaPreviewOverlay.classList.contains('d-none')) {
            closeMediaPreview();
        }
    });
...
function createFileRow(file) {
    ...
    dateCell.textContent = file.isParent ? '' : formatDate(new Date(file.modifiedTime));
    ...
    if (!file.isParent) {
        ...
        // 重命名按钮
        const renameBtn = createActionButton('重命名', 'bi-pencil', () => showRenameModal(file));
        actionsDiv.appendChild(renameBtn);

        // 删除按钮
        const deleteBtn = createActionButton('删除', 'bi-trash', () => deleteFile(file.path));
...
function getFileIconClass(file) {
    ...
    // 代码文件
    if (['js', 'html', 'css', 'php', 'py', 'java', 'c', 'cpp', 'h', 'cs', 'rb', 'go', 'ts', 'jsx', 'vue'].includes(ext)) {
        ...
        if (isImage || isVideo) {
            ...
            try {
                ...
                if (data.type === 'text') {
                    ...
                    // 设置下载链接
                    downloadFileBtn.href = `/api/file/download?path=${encodeURIComponent(file.path)}`;
                    ...
                    previewAudio.onerror = (e) => {
                        ...
                        unsupportedFileViewer.querySelector('h5').textContent = '音频加载失败';
...
async function searchFiles(query) {
    try {
        ...
        // 更新状态信息
        statusInfo.textContent = `搜索 "${query}" 的结果: ${data.results ? data.results.length : 0} 个项目`;
...
function createSearchResultRow(file, query) {
    ...
    // 修改日期
    const dateCell = document.createElement('td');
    dateCell.textContent = file.modifiedTime ? formatDate(new Date(file.modifiedTime)) : '';
    row.appendChild(dateCell);
    ...
    // 打开所在文件夹按钮
    if (file.path) {
        const openFolderBtn = createActionButton('打开所在文件夹', 'bi-folder', () => {
